...
const Select: React.FC<SelectProps> = ({ label, id, helpText, className = '', options, children, ...rest }) => {
  ...
  return (
    <div>
      ...
      <select
        id={selectId}
        className={`w-full px-3 py-2 border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent bg-background-secondary text-foreground ${className}`}
        {...rest}
      >
